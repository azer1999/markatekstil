(function ($) {
    ...
    $('.cd-trigger').on('click', function (event) {
        ...
        var product = $(this).closest('.single-grid-product, .single-list-product'),
            product_title = product.find('.title a'),
            product_brief = product.find('.brief'),
            product_price = product.find('.price'),
            product_badge = product.find('.product-badge-wrapper'),
            product_add_to_cart = product.find('.add-to-cart')
        $('body').addClass('overlay-layer');
        var qv = $("#qv-1")
        qv.find('.selected').html(`<img src='${slectedImageUrl}' alt="Product 2">`)
        qv.find('.item-title').text(product_title.text())
        qv.find('.price').html(product_price.html())
        qv.find('.description').html(product_brief.html())
        qv.find('.product-badge-wrapper').html(product_badge.html())
        qv.find('.add-to-cart').attr('data-product-id',product_add_to_cart.attr('data-product-id'))
    ...
    function resizeQuickView() {
        ...
        console.log(quickViewTop)
        console.log(quickViewLeft)
        $('.cd-quick-view').css({
            "top": quickViewTop,
            "left": quickViewLeft,
        });
